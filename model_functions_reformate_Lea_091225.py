...
def carbon_cycle (y,t0,W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas):
    ...
    for jk in range(0,max_temp_iter):
        ...
        if lum_vary=="y":
            ...
            if options_array[2]==1: # Check to see if methane option is on. If yes, use methane climate models:
                ...
                #w1 = 1/(1+numpy.exp(-15*(t0/1e9-0.541)))-1/(1+numpy.exp(-15*(t0/1e9-2.5))) # 1 in the Proterozoic, 0 elsewhere original
                ...
                #adjust w factors to make a spike in methane in proterozoic-> short jump to high methane model and back down again -> use calculations for low and high methane
                print('lenghts of methane release is', r_methane, 'Ga from 0.7 on' )
    ...
    #adjust weathering flux to change in value. current factor of -150 works but does not give a sufficent strong decrease-> needs another scaling factor
    scaling=1
    #scaling like does does not mean increase by a factor of xxx-> changes the exponential functions
    k_w_adj=k_w #*( 1+(scaling*(1/((1+numpy.exp(-150*(t0/1e9-0.7))))-1/(1+numpy.exp(-150*(t0/1e9-0.75))))))
    #print('t0 is', t0, 'k_w_adj', k_w_adj, 'k_w is', k_w)
    #Continetnal silicate weathering flux
    F_silic=k_w_adj*biology_modifier*land*(EE_ppCO2_o/ppCO2_o)**climp*numpy.exp((T_surface_diff)/tdep_weath) # equation 1
