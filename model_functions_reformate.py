...
def Forward_Model(W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,fpel,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas):
    ...
    for i in range(0,len(time)): 
        ...
        pl1=carbon_cycle(carbon_solution[i,:],time[i],W,F_outgass,n,climp,tdep_weath,None,None,None,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas, F_sil0) #ocean
    ...
    res_change2=(carbon_solution[max_el,0]-carbon_solution[2,0])*Mo+(carbon_solution[max_el,2]-carbon_solution[2,2])*Mp # alternative way of calculating final - initial carbon abundances
    
    imbalance=(flux_bal-res_change)/(carbon_solution[max_el,0]*Mo) # difference between integrated flux balance and final-initial abundances (should be zero).

    # return selected carbon cycle outputs to main_code_parallel.py
    return [carbon_solution[:,0],carbon_solution[:,1],carbon_solution[:,2],carbon_solution[:,3],time,pH_array_o,CO2_array_o,pH_array_p,CO2_array_p,Ca_array_o,Ca_array_p,CO3_array_o,CO3_array_p,HCO3_array_o,HCO3_array_p,omega_o,omega_p,Tsurf_array,Tdeep_array,Fd_array,Fs_array,Precip_ocean_array,Precip_pore_array,spread_ar,volc_array,T_pore_space_ar],imbalance 
...
def system_of_equations(y,t0,W,F_outgass,n,climp,tdep_weath,_mod_sea,_alt_frac,_Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas, mod_Si_weathering_rate):
    """
    Arguments:
        y: Array for the four previous solutions to the system of differential equations, in the order
        dCo, dAo, dCp, dAp. This must be an array for the odeint solver.

    Result:
        Array for the four solutions to the system of differential equations, in the order
        dCo, dAo, dCp, dAp. This must be an array for the odeint solver.
        mod_Si_weathering_rate: Modern day rate of silicate weathering in TMol/y.
    """

    global pK1,pK2,H_CO2,salt,Pore_mod,T_surface0
    global landf_vary,lum_vary,salt0,k_c,k_o,k_r,Mp,ppCO2_o,Mo,ko1,ko2
...
def carbon_cycle (y,t0,W,F_outgass,n,climp,tdep_weath,_mod_sea,_alt_frac,_Mp_frac,
                  lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,
                  mod_Si_weathering_rate):
    """
    Arguments:
        y: Array for the four previous solutions to the system of differential equations, in the order
        dCo, dAo, dCp, dAp. This must be an array for the odeint solver.
    ...
    # Continental silicate weathering flux
    # print(f't={t0/1e6:.2f}Ma\tSpike={spike}\tScale={scale}')
    F_silic=mod_Si_weathering_rate*scale*biology_modifier*land*(EE_ppCO2_o/ppCO2_o)**climp*numpy.exp((T_surface_diff)/tdep_weath) # equation 1
